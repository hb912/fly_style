  ...
  if (id) {
    try {
      ...
      product.size.map((data) => {
        ...
        const liEl = `사이즈<input class='input sizeField' type="text" value=${data.name}>&nbsp&nbsp&nbsp&nbsp&nbsp수량<input type="Number" class='input quantityField' value=${data.stock}>`;
        list.innerHTML = liEl;
    ...
    deleteBtn.className = "button is-danger is-fullwidth";
...
function sizeSubmitEvent(e) {
  ...
  const list = document.createElement("li");
  const liEl = `사이즈<input class='input sizeField' type="text"/>&nbsp&nbsp&nbsp&nbsp&nbsp수량<input type="Number" class='input quantityField'/>`;
  list.innerHTML = liEl;
  list.classList.add("sizeList");
  outputdata.appendChild(list);
}
...
async function submitEvent(e) {
  ...
  for (let i = 0; i < sizeFields.length; i++) {
    if (sizeFields[i].value !== "" && Number(quantityFields[i].value) > 0) {
      size.push({
        name: sizeFields[i].value,
        stock: Number(quantityFields[i].value),
      });
    }
  }
  ...
  try {
    ...
    await Api.formDataPost("/api/product/uplodimg", formdata);
